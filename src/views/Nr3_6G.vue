<template>
  <div class="nr">
    <page-header backUrl="grid" title="3G-6G频点"></page-header>
    ...
    <van-cell-group title="输出" class="output-container">
      <van-cell title="arfcnValueNr" :value="computedArf" />
...
export default {
  ...
  data() {
    return { ctype: 1, a: 3510, b: 100, c: 0.03, d: 273, e: 0, f: 48 };
  },
  computed: {
    computedM() {
      if (this.ctype == 1) {
    ...
    computedSSBN() {
      ...
      if (this.f == 24) {
        return this.roundUp((g + 12 * 10 * this.c - 3000) / 1.44);
      } else if (this.f == 48) {
        return this.roundUp(
          (g + 12 * 12 * this.c + 12 * 10 * this.c - 3000) / 1.44
        );
      }
      return 0;
    },
    ...
    computedSSBAbs() {
      return this.roundUp(600000 + Math.round((this.computedH - 3000) / 0.015));
    },
    computedPointAAbs() {
      return this.roundUp(600000 + (this.computedPointA - 3000) / 0.015);
    }
  },
  methods: {
    roundUp(x) {
